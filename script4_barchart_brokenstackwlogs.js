...
Promise.all([
  ...
  colorScaleSVG7 = d3.scaleOrdinal()
    .range(["#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3", "#fdb462", "#b3de69", "#fccde5", "#d9d9d9", "#bc80bd", "#ccebc5", "#ffed6f"]);

  // Append stacked bars for household data in SVG7 with categorical color
  const barsHousehold = svg7.selectAll(".bar-household")
    .data(stack(combinedData.filter(d => d.Set === 'Yearly household expenses')))
    .enter().append("g")
    .attr("class", d => `bar-household ${d.key.toLowerCase()}`)
    .selectAll("rect")
    .data(d => {
      console.log("Stacked Data:", d);
      return d;
    })
    .enter().append("rect")
    .attr("x", d => {
      console.log("X Value:", xScale7(d.data.Set));
      return 50 + xScale7(d.data.Set);
    })
    ...
    .attr("height", d => {
      console.log("Height Value:", d[0] - d[1]);
      return yScale7(d[0]) - yScale7(d[1]);
    })
