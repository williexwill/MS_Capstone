...
// Define the initial dataset (BLS_2021_clean.csv)
let currentDataset = "data/BLS_2021_clean.csv";

// Load the initial dataset
d3.csv(currentDataset, d => ({
  Category: d.Category,
  Spending: +d.Spending
})).then(data => {
  // Create a single stacked bar for "Yearly household expenses"
  const initialStack = d3.stack()
    .keys(data.columns.slice(1)) // Assuming the CSV has a 'Category' column and other columns for each category's value
    .order(d3.stackOrderNone)
    .offset(d3.stackOffsetNone)(data);

  // Set up scales and axes for SVG7
  const xScale7 = d3.scaleBand()
    .domain(["Yearly household expenses"]) // Initial label
    .range([0, 600])
    .padding(0.1);

  const yScale7 = d3.scaleLinear()
    .domain([0, 100000]) // Initial max domain for y axis
    .range([400, 0])
    .nice();
  ...
  // Append initial stacked bars for SVG7 with categorical color
  svg7.selectAll(".bar-stack")
    .data(initialStack)
    .enter().append("g")
    .attr("class", "bar-stack")
    .attr("fill", d => colorScaleSVG7(d.key))
    .selectAll("rect")
    .data(d => d)
    .enter().append("rect")
    .attr("x", d => 50 + xScale7("Yearly household expenses"))
    .attr("y", d => yScale7(d[1]))
    .attr("height", d => yScale7(d[0]) - yScale7(d[1]))
    .attr("width", xScale7.bandwidth());

  // Append a button for comparison
  svg7.append("rect")
    .attr("x", 700)
    .attr("y", 10)
    .attr("width", 80)
  ...
  function handleCompareClick() {
    // Load the second dataset (StLou_clean.csv)
    d3.csv("data/StLou_clean.csv", d => ({
      Department: d.Department,
      Spending: +d.Spending
    })).then(secondData => {
      // Change the scale of the y axis with a transition
      yScale7.domain([0, 1000000000]).nice();

      // Transition for the existing stacked bars
      svg7.selectAll(".bar-stack")
        .selectAll("rect")
        .transition()
      ...
      // Append new stacked bars for the second dataset with a transition
      const newStack = d3.stack()
        .keys(secondData.columns.slice(1))
        .order(d3.stackOrderNone)
        .offset(d3.stackOffsetNone)(secondData);

      svg7.selectAll(".bar-stack-new")
        .data(newStack)
        .enter().append("g")
        .attr("class", "bar-stack-new")
        .attr("fill", d => colorScaleSVG7(d.key))
        .selectAll("rect")
        .data(d => d)
        .enter().append("rect")
        .attr("x", d => 50 + xScale7("Yearly municipal expenses"))
        .attr("y", d => yScale7(d[1]))
        .attr("height", d => yScale7(d[0]) - yScale7(d[1]))
        .attr("width", xScale7.bandwidth())
