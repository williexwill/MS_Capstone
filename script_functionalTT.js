...
d3.csv("data/BigNumbers_Clean.csv", d => d).then(data => {
  ...
  data.forEach(d => {
    const xPosition = d.Number % 800 + margin.left + tooltipMargin;
    const yPosition = Math.floor(d.Number / 800) + margin.top + tooltipMargin;

    // Create a line from the pixel/rectangle to the tooltip
    svg4.append("line")
      .attr("class", "tooltip-line")
      .attr("x1", d.Number % 800)
      .attr("y1", Math.floor(d.Number / 800))
      .attr("x2", xPosition)
      .attr("y2", yPosition)
      .attr("stroke", "white");

    // Append foreignObject for the dynamic tooltip
    const foreignObject = svg4.append("foreignObject")
      .attr("class", "dynamic-tooltip")
      .attr("x", xPosition)
      .attr("y", yPosition)
      .attr("width", 200) // Set a fixed width, adjust as needed;
